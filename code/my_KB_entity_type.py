...
dy_filename = "./data/MS/book_data.ttl"
# dy_filename = "./data/MS/model_data.ttl"

prox_graph_file = "./data/MS/test_graph"
graph = Graph()
...
#         {Q} <http://example.org/has_label> ?label .
#     }}
#     """
...
#     except Exception:
#         return []
def getLabel(Q, graph):
    labels = []
    subj = URIRef(Q)
    # pred = URIRef("http://example.org/has_label")
    pred = URIRef("http://book.org/label")
...
def getRDFData(o,graph):
    if str(o).startswith('http://book.org/'):
    # if str(o).startswith('http://example.org/'):

        # Q_entity = "<"+o+">"
        # print(Q_entity)
        # data_type = getRdfType(Q_entity)
        data_type = getLabel(o,graph)
    else:
        data_type = [dataType(str(o))]
...
for s,p,o in graph:
    i += 1
    # print(i)
    s, s_data_type = getRDFData(str(s),graph) # change data type
    o, o_data_type = getRDFData(str(o),graph)
    # print(o, o_data_type)
    # print(s, p, o)
...
with open('./data/MS/typeset1.txt', 'w') as f:
    f.write(','.join(list(typeset1)))
with open('./data/MS/typeset2.txt', 'w') as f:
